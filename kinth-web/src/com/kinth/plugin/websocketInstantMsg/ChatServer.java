package com.kinth.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("WUadminqq313388925")){
			this.userjoin(message.replaceFirst("WUadminqq313388925", ""),conn);
		}if(null != message && message.startsWith("LeaveWUadminqq313388925")){
			this.userLeave(conn);
		}if(null != message && message.contains("wuadmin886")){
			String toUser = message.substring(message.indexOf("wuadmin886")+10, message.indexOf("wuwuadmin888"));
			message = message.substring(0, message.indexOf("wuadmin886")) +"[私信]  "+ message.substring(message.indexOf("wuwuadmin888")+12, message.length());
